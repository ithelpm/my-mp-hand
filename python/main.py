...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        default=0.5)
    # Finding the camera ID can be very reliant on platform-dependent methods. One common approach is to use the fact
    # that camera IDs are usually indexed sequentially by the OS, starting from 0. Here, we use OpenCV and create a
    # VideoCapture object for each potential ID with 'cap = cv2.VideoCapture(i)'. If 'cap' is None or not
    # 'cap.isOpened()', it indicates the camera ID is not available.
    parser.add_argument(
